...
// All test functions must start with Test* and take a pointer to testing.T
// to be valid for the test suite to run.
func TestAWSService_GetAWSCredentials(t *testing.T) {
...
# aws_secret_access_key is missing`
	...
	tests := []struct {
		...
		{
			name:           "valid profile with complete credentials",
			profile:        "default",
		...
		{
			name:           "valid profile with complete credentials - int",
			profile:        "int",
		...
		{
			name:          "profile with missing secret key",
			profile:       "incomplete",
			expectError:   true,
			errorContains: "AWS credentials not found for profile 'incomplete'",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			accessKey, secretKey, err := awsService.GetAWSCredentials(tt.profile)
			...
			if tt.expectError {
				...
				if accessKey != tt.expectedKey {
					t.Errorf("Expected access key '%s', got '%s'", tt.expectedKey, accessKey)
				}
				if secretKey != tt.expectedSecret {
					t.Errorf("Expected secret key '%s', got '%s'", tt.expectedSecret, secretKey)
				}
			}
		})
	}
